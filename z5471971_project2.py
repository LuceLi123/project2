...
def read_prc_csv(tic):
    ...
    """
    # <COMPLETE THIS PART>
    df = pd.read_csv(os.path.join(cfg.DATADIR, f"{tic}_prc.csv"), index_col=0)
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    data_list = [cfg.standardise_colnames(read_prc_csv(tic)[[prc_col]].rename(columns={prc_col: tic}))
                 for tic in tickers]
    return pd.concat(data_list, axis=1, join='outer')
...
def mk_aret_df(ret_df):
    ...
    """
    # <COMPLETE THIS PART>
    ret_df = ret_df.subtract(ret_df['mkt'], axis=0).drop('mkt', axis=1)
    return ret_df
